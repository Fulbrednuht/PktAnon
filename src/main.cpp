...
//	StatisticEngine* statistic = new StatisticEngine();
	...
	// IP- and MAC-Ranges
	//
	uint32_t 	ipv4Replace			= config->getIPv4ReplaceIp		();
	int 		v4subnet 			= config->getIPv4ReplaceSubnet	();
	uint32_t 	ipv4MapTo			= config->getIPv4MapToRange		();
	int 		ipv4Index 			= config->getIPv4MapToIndex		();

	uint64_t 	macMapTo 			= config->getMacMapToRange		();
	int 		macIndex			= config->getMacMapToIndex		();
    unordered_set<uint64_t> macs 	= config->getMacReplaceSet		();

    bitset<128>	ipv6			 	= config->getIPv6ReplaceIp		();
	int 		v6subnet			= config->getIPv6Subnet			();

	assertExit(8 * ipv4Index < v4subnet, "IPv4 Map to Range is too short");
	assertExit(8 * macIndex < macs.size(), "MAC Map to Range is too short");
	assertExit(v4subnet >= 8, "IPv4 Replace Range is too big. Please choose a subnet between 8 and 31");
	assertExit(v6subnet >= 24, "IPv6 Replace Range is too big. Please choose a subnet between 24 and 127");
 // TODO: fehlerhafte IPv6 Überprüfung in mapAddress() korrigieren, zu große IPv4 & IPv6 startaddresse bei entsprecheneder subnetmask (overflow) abfangen
	/*union {
		bitset<128> address = 0;
		uint64_t part[2];
	} debugVar;
	debugIPv6.address = ipv6;

	std::cout << "Endianess: "  << (Utils::isBigEndian() ? "big" : "little") << "\n" << std::hex << "IPv4 Replace IP: "<< ipv4Replace << " \tIPv4 Replace Subnet: "
			<< v4subnet <<"\nipv4MapTo: " << ipv4MapTo << " \tIPv4 MapTo Index: " << ipv4Index
			<< "\nMACs size: " << macs.size()  << " \tMAC map to: " << macMapTo  << " \t MAC map to Index: " << macIndex
			<<"\nIPv6 Part1: " << debugIPv6.part[1] << " \tIPv6 Part2: " << debugIPv6.part[0] <<  " \t IPv6: " << ipv6 << " \tIPv6 subnet: " << v6subnet << std::endl;
*/

	AddressMapping* am = AddressMapping::getInstance();
	am->initIPv4(ipv4Replace, v4subnet, ipv4MapTo, ipv4Index);
	am->initIPv6(ipv6, v6subnet);
	am->initMAC(macs, macMapTo, macIndex);
	...
	assertExit (sizeof (int64_t)		== 8, "invalid data type size for long");
	assertExit (sizeof (uint64_t)		== 8, "invalid data type size for unsigned long");
